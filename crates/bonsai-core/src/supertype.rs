...
/// Provides supertype relationships from the released grammar crate's
/// embedded `node-types.json` metadata.
...
impl NodeTypesProvider {
    /// Resolve the released grammar's embedded node-type metadata to kind IDs.
    pub fn new(language: &Language, lang_name: &str) -> Self {
        let entries: Vec<NodeTypeEntry> = crate::languages::get_node_types_json(lang_name)
            .map(|json| {
                serde_json::from_str(json).unwrap_or_else(|error| {
                    panic!("invalid node-types.json for {lang_name}: {error}")
                })
            })
            .unwrap_or_default();
        let mut kind_to_supertypes: HashMap<u16, Vec<u16>> = HashMap::new();
        ...
        for entry in entries {
            let Some(subtype_entries) = entry.subtypes else {
                continue;
            };
            let supertype_id = language.id_for_node_kind(&entry.type_name, true);
            if supertype_id == 0 {
                continue;
            }

            let mut subtypes = Vec::new();
            for subtype in subtype_entries.into_iter().filter(|subtype| subtype.named) {
                let subtype_id = language.id_for_node_kind(&subtype.type_name, true);
                if subtype_id == 0 {
                    continue;
                }
...
/// Provides supertype relationships hand-declared in grammars.toml.
///
/// This is the escape hatch for grammars whose `Language::supertypes()`
/// and `node-types.json` are both empty (e.g., tree-sitter-clojure).
/// Maintainers declare mappings in the `[language.supertypes]` table:
///
/// ```toml
/// [[language]]
/// name = "clojure"
/// grammar = "grammars/tree-sitter-clojure"
/// extensions = ["clj", "lg", ...]
/// src = "src"
///
/// [language.supertypes]
/// _atom = ["sym_lit", "kwd_lit", "str_lit"]
/// ```
///
/// **Soundness:** Only declare a supertype whose subtypes are
/// grammatically interchangeable in EVERY syntactic position.
/// Declaring loose families causes the Unwrap transform to produce
/// candidates that parse but are semantically broken (e.g., replacing
/// a function call with a bare identifier).
///
/// Synthetic supertype names that start with `_` won't collide with
/// real node kinds. They exist purely as a join point for the
/// compatibility relation; the runtime never tries to materialize a
/// node OF the supertype kind.
pub struct ConfigSupertypeProvider {
    kind_to_supertypes: HashMap<u16, Vec<u16>>,
    supertype_to_subtypes: HashMap<u16, Vec<u16>>,
}

impl ConfigSupertypeProvider {
    /// Build from the static mappings generated by build.rs from
    /// grammars.toml. Resolves type names to kind IDs using the
    /// Language; unknown names are silently skipped (so a typo in
    /// grammars.toml doesn't crash the runtime — it just makes the
    /// declaration a no-op until fixed).
    ///
    /// Synthetic supertype names (those not present in the grammar's
    /// kind table) get auto-assigned synthetic IDs starting from a
    /// high offset so they don't collide with real kind IDs. This lets
    /// the Unwrap transform use them purely as join points for the
    /// compatibility relation.
    pub fn new(language: &Language, lang_name: &str) -> Self {
        let mappings = crate::languages::get_config_supertypes(lang_name);
        let mut kind_to_supertypes: HashMap<u16, Vec<u16>> = HashMap::new();
        let mut supertype_to_subtypes: HashMap<u16, Vec<u16>> = HashMap::new();

        // Synthetic supertype IDs start above the grammar's real kind
        // space. tree-sitter kind IDs are u16 and grammars rarely
        // exceed a few thousand; using 60000+ keeps us safely above
        // any real ID.
        let mut next_synthetic_id: u16 = 60000;

        for &(supertype_name, subtype_names) in mappings {
            // Try to resolve the supertype as a real grammar kind first.
            // If it doesn't exist (typical case for `_atom`-style
            // synthetic names), assign a synthetic ID.
            ...
            for &subtype_name in subtype_names {
                ...
                if subtype_id == 0 {
                    // Subtype must be a real grammar kind. Unknown
                    // names skipped (likely a typo in grammars.toml).
                    continue;
                }
                subtypes.push(subtype_id);
                kind_to_supertypes
...
impl SupertypeProvider for ConfigSupertypeProvider {
    fn supertypes_for(&self, kind_id: u16) -> Vec<u16> {
        self.kind_to_supertypes
            .get(&kind_id)
            .cloned()
...
mod tests {
    ...
    // ConfigSupertypeProvider — hand-declared via grammars.toml.

    #[test]
    fn test_config_provider_clojure_has_no_supertypes_by_default() {
        // The Clojure grammar entry ships without a [language.supertypes]
        // table (intentional — see Option A soundness discussion).
        // Until concrete sound supertypes are identified, this provider
        // should return nothing for Clojure.
        let lang = languages::get_language("clojure").unwrap();
        let provider = ConfigSupertypeProvider::new(&lang, "clojure");
        assert!(
            !provider.has_supertypes(),
            "Clojure ships without config-declared supertypes by default"
        );
    }

    #[test]
    fn test_config_provider_python_augments_release_metadata() {
        // The pure 0.25.0 release keeps expression_statement as a wrapper.
        // Bonsai owns the additional compatibility relationship rather than
        // depending on an unreleased or patched parser revision.
        let lang = languages::get_language("python").unwrap();
        let provider = ConfigSupertypeProvider::new(&lang, "python");
        let assignment = lang.id_for_node_kind("assignment", true);
        let expression_statement = lang.id_for_node_kind("expression_statement", true);

        assert!(provider.has_supertypes());
        assert!(
            provider
                .supertypes_for(assignment)
                .contains(&expression_statement),
            "assignment should inherit Bonsai's expression_statement compatibility"
        );
    }

    #[test]
    fn test_config_provider_unknown_language_is_empty() {
        let lang = languages::get_language("python").unwrap();
        let provider = ConfigSupertypeProvider::new(&lang, "nonexistent");
        assert!(
            !provider.has_supertypes(),
            "Unknown language should have no config supertypes"
        );
    }

    #[test]
    fn test_chain_provider_includes_config_provider() {
        // The full chain (used by the CLI) should be Empty for Clojure
        // today — none of Language API, node-types.json, or
        // grammars.toml has supertypes declared.
        let lang = languages::get_language("clojure").unwrap();
        let api = LanguageApiProvider::new(&lang);
        let ntp = NodeTypesProvider::new(&lang, "clojure");
        let cfg = ConfigSupertypeProvider::new(&lang, "clojure");

        let chain = ChainProvider::new(vec![Box::new(api), Box::new(ntp), Box::new(cfg)]);

        // Every Clojure node kind should have an empty supertype list.
        // Spot-check a few common kinds:
        let list_kind = lang.id_for_node_kind("list_lit", true);
        let sym_kind = lang.id_for_node_kind("sym_lit", true);
        if list_kind != 0 {
            assert!(
                chain.supertypes_for(list_kind).is_empty(),
                "list_lit should have no supertypes from any provider"
            );
        }
        if sym_kind != 0 {
            assert!(
                chain.supertypes_for(sym_kind).is_empty(),
                "sym_lit should have no supertypes from any provider"
            );
        }
    }
