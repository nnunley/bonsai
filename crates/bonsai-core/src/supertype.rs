...
mod tests {
    ...
    #[test]
    fn test_config_provider_python_has_no_supertypes() {
        // Python doesn't need config-declared supertypes; the grammar
        // declares its own.
        let lang = languages::get_language("python").unwrap();
        let provider = ConfigSupertypeProvider::new(&lang, "python");
        assert!(
            !provider.has_supertypes(),
            "Python ships without config-declared supertypes"
        );
    }
